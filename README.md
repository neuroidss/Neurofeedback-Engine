...
### 🧬 Genesis Driver (Recursive World)
An autopoietic narrative engine driven by **Recursive Language Models (RLM)**. It simulates a conscious Game Master that evolves with you.
*   **Theory of Mind:** The GM explicitly models your psychological state based on your actions and the lore, displaying its analysis of *you* in real-time.
*   **The Crossroads:** A high-pressure decision phase with a countdown. Hesitation triggers a default consequence defined by the narrative flow.
*   **Narrative Filter:** Your inputs (voice/text) are not accepted literally; they are interpreted and re-written by the engine to ensure strict lore adherence before the world reacts.
*   **Infinite Context:** The engine writes its own code to recursively summarize and compress history, allowing for timelines that span infinite turns without memory loss.
